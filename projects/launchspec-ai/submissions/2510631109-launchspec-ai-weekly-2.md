...
满足：本地检查通过（退出码 0）、UAT 覆盖主路径 + 2 条以上错误路径、密钥扫描无真实凭据、README 可让评审者独立启动、能解释被推翻的架构决策。

未满足：F-1 独立真人结对 UAT 未完成；F-2 提交粒度缺口（承接 Gate 2）。
...
5. 未完成独立真人结对 UAT（`process/uat/human-pair-uat-template.md` 仍为空模板）。
6. 未保留测试先行的红绿提交序列。
